...
var key = 'progressiveDelivery.saasPromotionsJson';
function createRouter(options) {
    return __awaiter(this, void 0, void 0, function () {
        var logger, config, router;
        return __generator(this, function (_a) {
            logger = options.logger, config = options.config;
            router = (0, express_promise_router_1.default)();
            router.use(express_1.default.json());
            router.get('/health', function (_, response) {
                logger.info('PONG!');
                response.json({ status: 'ok' });
            });
            router.get('/topo', function (_, response) {
                response.sendFile(config.getString(key));
            });
            router.use((0, backend_common_1.errorHandler)());
