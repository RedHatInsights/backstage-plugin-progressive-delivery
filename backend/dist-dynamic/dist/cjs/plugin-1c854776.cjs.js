...
async function createRouter(options) {
  ...
  router.get("/topo", (_, response) => {
    backendCommon.loadBackendConfig({ logger, argv: [] }).then((config) => {
      ...
      console.log("SMR KEYS", config.keys());
      console.log("SMR KEYS", config.get("progressive-delivery"));
      if (config.has(key)) {
        response.sendFile(config.getString(key));
      } else {
        console.log("ERROR", config.keys());
        console.log("ERROR", config.get("progressive-delivery"));
      }
    }).catch((error) => {
...
//# sourceMappingURL=plugin-1c854776.cjs.js.map
